...
# ╔═╡ 6e59414e-937c-429f-9397-b188d77391c7
using Distributions, FHist, StatsBase, CairoMakie

# ╔═╡ c543d1aa-3fd9-4cf9-bb97-a1903bbefdaa
Normal() # default to unit gaussian

# ╔═╡ 9dc19c6d-aef4-44e6-85ad-dc207f903472
# you can also randn(1000) which is built-in
data = rand(Normal(), 1000)

# ╔═╡ 46b133cd-8987-48eb-8ef1-579d4cedef01
observed_hist = Hist1D(data, -3:0.1:3)

# ╔═╡ b1235d55-4a02-496c-9247-2bac9f512242
norm_hist = normalize(observed_hist); # by default normalize to a proper pdf

# ╔═╡ 9cf2648c-ebda-49bd-af28-dae08560b922
μ, σ = mean(norm_hist), std(norm_hist)

# ╔═╡ b8d0d6a6-2b7d-4c8a-8182-f32f99f56b5e
begin
	lines(
		Normal(μ, σ);
		axis=(;ylabel="a.u.", xlabel="Observable"),
		label="Best fit of Normal model\n μ=$(round(μ; digits=3)), σ=$(round(σ; digits=3))"
	)

	scatter!(norm_hist; color=:black, label="Observations (normalized)")
	errorbars!(norm_hist)
	axislegend()
	current_figure()
end
...
manifest_format = "2.0"
project_hash = "53b3a88c28cb2fc4ed4ccc9ed4e8b59f562b9d80"
...
git-tree-sha1 = "b6becb89dbb93be91658947d606bc145270248df"
